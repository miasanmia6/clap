...
def print_graph(tensor):
    ...
    if tensor.grad_fn is not None:
        ...
        print_graph(tensor.grad_fn.next_functions[0][0])
...
def _get_base_text_features(cfg, classnames, clip_model, text_encoder, pretrained_projection=None):
    ...
    # 保存到文件
    torch.save({
        "base_text_features": text_embeddings_avg,
        "classnames": classnames,
        "dataset": dataset
    }, 'base_text_features_eurosat.pt')
    print('已经将base_text_features保存')
...
class CustomCLIP(nn.Module):
    def __init__(self, cfg, classnames, clip_model):
        ...
        # 得到 CLIP 模型输出的文本（类别）特征
        if cfg.TRAINER.BayesADAPTER.ENHANCED_BASE == "none":
        ...
        self.to(clip_model.visual.conv1.weight.device)  # 与CLIP主模型设备一致
    ...
    def forward(self, image, return_features=False):
        ...
        #     logits = self.forward_lp(image_features)
            ...
            logits = self.forward_bays(image_features)
    ...
    def forward_features(self, features):
        ...
        #     logits = self.forward_lp(features)
            ...
            logits = self.forward_bays(features)
    ...
    def forward_bays(self, features):
        ...
        # Get trained prototype
        # 如果是将类原型向量视为随机变量，此时得到的prototypes大小为[n_samples,n_classes,feat_dim]
        # with torch.no_grad():  # 采样时不需梯度
        #     prototypes = self.adapter()
        ...
        # 对每个采样计算 logits，大小为 [n_samples, n_features, n_classes]
        # 计算所有采样样本的logits [B, S, C]
        all_logits = torch.einsum('bd,scd->bsc', image_features_norm, prototypes_norm) * logit_scale

        # 均值 [B, C]
        logits_mean = all_logits.mean(dim=1)
        # 方差 [B, C]
        logits_var = all_logits.var(dim=1, unbiased=False)  # 无偏估计设为False

        return logits_mean  # [B, C]
...
class AdapterMethod(nn.Module):
    def __init__(self, cfg, clip_model, base_text_features):
        ...
        self.logit_scale = clip_model.logit_scale
        self.initialization = cfg.TRAINER.BayesADAPTER.INIT
        self.apply_constraint = cfg.TRAINER.BayesADAPTER.CONSTRAINT
        self.distance = "KL" # L2
        ...
        self.kl_weight = 0 # 初始权重为0
        self.anneal_start_epoch = 20  # 延迟到第20个epoch开始退火
        self.total_epochs = 300  # 总epoch数
        self.anneal_rate = 1.0 / (self.total_epochs - self.anneal_start_epoch)  # 调整退火速率
        ...
        elif "GAUSSIAN_PER_CLASS" in self.initialization: # Bayesian_CLAP cvpr2025
            print("Using per-class Gaussian initialization")
            self.init_gaussian_per_class()
        else:
    ...
    def init_gaussian_per_class(self):
        """变分分布初始化：
        - 后验均值μ：从base_text_features开始学习
        - 后验协方差Σ：每个类独立对角协方差（初始=0.01）
        """

        n_classes, feat_dim = self.base_text_features.shape

        self.variational_mu = nn.Parameter(self.base_text_features.clone())


        self.variational_log_sigma = nn.Parameter(torch.full((n_classes, feat_dim), math.log(0.01)))


        # 先验参数（固定）
        self.register_buffer("prior_mu", self.base_text_features.clone().to('cuda:0'))
        self.register_buffer("prior_sigma", torch.full((n_classes, feat_dim), 0.01).to('cuda:0'))  # sqrt(0.01)

    def kl_divergence(self):
        """计算KL(q||p)其中：
        q = N(variational_mu, variational_sigma^2)
        p = N(prior_mu, prior_sigma^2)
        """
        # 确保所有计算在同一设备上
        posterior_sigma = torch.exp(self.variational_log_sigma) + 1e-8  # 防止σ=0
        prior_sigma = self.prior_sigma.to(self.variational_mu.device) + 1e-8
        prior_mu = self.prior_mu.to(self.variational_mu.device)

        # 使用数值稳定的公式（避免直接计算 log(σ_p / σ_q)）
        kl_per_dim = (
                (posterior_sigma ** 2 + (self.variational_mu - prior_mu) ** 2) / (2 * prior_sigma ** 2)
                - 0.5
                - torch.log(posterior_sigma) + torch.log(prior_sigma)  # 使用保护后的σ
        )

        kl = kl_per_dim.sum()
        return torch.clamp(kl, max=1e4)  # 限制KL最大值
    ...
    def init_tipadapter(self, features_train, labels_train):
        self.cache_keys = nn.Parameter(features_train.clone(),device=self.device)
    ...
    def forward(self, n_samples=1):
        if "GAUSSIAN_PER_CLASS" in self.initialization:
            # 重参数化采样（保留梯度路径）
            epsilon = torch.randn(
...
class BayesADAPTER(TrainerXCostume):
    """General Adapter
    """
    def check_cfg(self, cfg):
        assert cfg.TRAINER.BayesADAPTER.PREC in ["fp16", "fp32", "amp"]
    ...
    def build_model(self):
        ...
        if cfg.TRAINER.BayesADAPTER.PREC == "fp32" or cfg.TRAINER.BayesADAPTER.PREC == "amp":
    ...
    #     self.optim = build_optimizer(self.model.adapter, cfg.OPTIM, param_groups=[
    #     {"params": self.model.adapter.variational_mu, "lr": cfg.OPTIM.LR * 0.001},
    #     {"params": self.model.adapter.variational_log_sigma, "lr": cfg.OPTIM.LR * 0.01},
    # ],)
        self.optim = build_optimizer(self.model.adapter, cfg.OPTIM)
        self.sched = build_lr_scheduler(self.optim, cfg.OPTIM)
        self.register_model("adapter", self.model.adapter, self.optim, self.sched)

        print("Optimizer parameters:", self.optim.param_groups[0].keys())
        # 输出应包含 'momentum' 键且值为0.9
        self.scaler = GradScaler() if cfg.TRAINER.BayesADAPTER.PREC == "amp" else None

    def train(self):
        self.set_model_mode("eval")

        # Feature extraction on test set
        self.labels_test, output_test, self.features_test = self.extract_features(partition="test")  # cuda:0
        print(f"Zero-Shot accuracy on test: " +
        ...
        if self.model.adapter.apply_constraint != "none" and self.model.adapter.apply_constraint != 'KL':
            print("Getting initial lagrangian multipliers for constraint formulation", end="\n")
            self.model.adapter.device = self.device
            print(self.labels_train.device)
            print(self.logits_zs.device)
    ...
    def forward_backward(self, features, labels):
        # 用来计算损失函数，反向传播
        prec = self.cfg.TRAINER.BayesADAPTER.PREC
        if prec == "amp":
            with autocast():
                # Cross-entropy loss
            ...
            # Cross-entropy loss
            # 如果将类原型向量设置为分从高斯分布的随机变量的话，这里得到的output是从变分分布中采样三次计算logits后平均的结果
            output = self.model.forward_features(torch.tensor(features).to(self.device))
            # Softmax cross-entropy
            loss_ce = F.cross_entropy(output, labels)
            # Constraint to zero-shot (CLAP)
            ...
            if self.model.adapter.apply_constraint != "none" and self.model.adapter.apply_constraint == 'KL':
                ...
                # 添加KL退火系数
                current_kl_weight = self.model.adapter.kl_weight
                loss = loss_ce + current_kl_weight * loss_kl
                print(f"loss_ce: {loss_ce.item()}")
                print(f"KL divergence: {loss_kl.item()}, weight: {current_kl_weight}")
            else:
                loss = loss_ce
            ...
            # print("Variational Mu:", self.model.adapter.variational_mu[0, 0].item())
            # print("Variational Log Sigma:", self.model.adapter.variational_log_sigma[0, 0].item())
            # print("Mu grad norm:", self.model.adapter.variational_mu.grad.norm().item())
            # print("Sigma grad norm:", self.model.adapter.variational_log_sigma.grad.norm().item())
        ...
        with torch.no_grad():
            ...
            probs = F.softmax(output_test, dim=-1)
            confidence = probs.max(dim=-1).values  # 每个样本的最大概率（置信度）
            predicted_classes = probs.argmax(dim=-1)  # 预测类别

            # 真实标签
            true_labels = self.labels_test.clone().detach().to(self.device)
            ...
            # Step 1: 设置 bins（10个置信度区间）
            confidence_bins = [(i / 10, (i + 1) / 10) for i in range(10)]  # [(0.0,0.1), (0.1,0.2), ..., (0.9,1.0)]
            ...
            for (lower, upper) in confidence_bins:
                ...
                bin_preds = predicted_classes[bin_mask]
                bin_true = true_labels[bin_mask]
                bin_conf = confidence[bin_mask]

                if len(bin_preds) > 0:
                    accuracy = (bin_preds == bin_true).float().mean().item()
                    mean_conf = bin_conf.mean().item()
                    abs_gap = abs(accuracy - mean_conf)

                    # 类别覆盖率
                    covered_classes = torch.unique(bin_preds).numel()
                    total_classes = torch.unique(true_labels).numel()
                    class_coverage = covered_classes / total_classes * 100

                    # ECE 计算（每个 bin 的样本比例 × gap）
                    bin_weight = len(bin_preds) / total_samples
                    ece_total += bin_weight * abs_gap

                    # AECE 计算（每个 bin 的平均 gap）
                    aece_total += abs_gap / 10  # 10 bins，等权重平均
            ...
            # Step 3: 转为 DataFrame 并添加 ECE 与 AECE 汇总行
            df_cov = pd.DataFrame(coverage_data)

            # 添加一行 ECE 与 AECE 的汇总信息
            summary_row = pd.DataFrame([{
                'confidence_bin': 'Summary (ECE/AECE)',
                'accuracy': None,
                'class_coverage': None,
                'num_samples': total_samples,
                'mean_confidence': f"ECE={ece_total:.4f}, AECE={aece_total:.4f}",
                'abs_gap': None
            }])

            df_cov = pd.concat([df_cov, summary_row], ignore_index=True)

            # Step 4: 保存 CSV
            df_cov.to_csv(f"{self.cfg.OUTPUT_DIR}/confidence_coverage_stats.csv", index=False)
    ...
    def extract_features(self, partition, reps=1, transforms=False):
        ...
        if "TipA" not in self.model.adapter.initialization:
            ...
            for rep in range(reps):
                for batch_idx, batch in enumerate(tqdm(data_loader)):
                    with torch.no_grad():
                        input, label = self.parse_batch_test(batch) # cuda:0
                        logits, features = self.model(input, return_features=True) # cuda:0
        ...
        return labels_ds.to(self.device), logits_ds.to(self.device), features_ds.to(self.device) # cuda:0
